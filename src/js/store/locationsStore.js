	...
	class Locations {
		...
		serializeCities(cities){
			...
			this.cities =  cities.reduce((acc, city)=>{
				let cityName = city.name || city.name_translations.en
		...
		getCityCodeByName(name){
			try{
				const city = Object.values(this.cities).find((item)=>{
					return  item.fullName.split(' ').join('') === name.split(' ').join('');
				});
				return city.code;
			}catch (err) {
		...
		getTicketObjectById(_id){
			...
			const ticket = 	this.lastSearch.filter((ticket) =>{
				return ticket.uniqueId === _id;
			});
			return ticket[0];
		}
		...
		createDataForAutocomplete(cities){
			...
			// console.log(cities);
			this.dataForAutocomplete =  Object.entries(cities).reduce((acc, [code, cityObj])=>{
		...
		async fetchTickets(params){
			let {origin,destination,depart_date,return_date,currency} = params
